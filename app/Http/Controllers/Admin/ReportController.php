...
class ReportController extends Controller
{
    ...
    $today = date('d-m-y');
    $order = DB::table('orders')->where('status',0)->where('day',$today)->get();
    ...
    public function TodayDelivery(){
        $today = date('d-m-y');
        $order = DB::table('orders')->where('status',3)->where('day',$today)->get();
    ...
    public function ThisMonth(){
        ...
        public function SearchByDate(Request $request){
            ...
            $total = DB::table('orders')->where('status',3)->where('day',$newdate)->sum('total');
           
            $order = DB::table('orders')->where('status',3)->where('day',$newdate)->get();
